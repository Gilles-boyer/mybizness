...
export default {
  methods: {
    addListGifts(list) {
      this.voucher.gifts = list;
      this.nextStep();
    },
    addPersonalization(data) {
      this.voucher.personalization = data;
      console.log(this.voucher.personalization)
      this.nextStep();
    },
    addDataForm(data) {
      this.voucher.customer = data.customer;
      this.voucher.beneficiary = data.beneficiary;
      this.voucher.shipping = data.shipping;
      this.nextStep();
    },
    confirmationAndPaiement() {
      ...
      this.voucher.gifts.forEach(element => tabIdGift.push(element.id))
      ...
      form.innerHTML += `<input type="text" name="payment[first_name]" value="${this.voucher.customer.firstName}"/>`;
      form.innerHTML += `<input type="text" name="payment[last_name]" value="${this.voucher.customer.lastName}"/>`;
      form.innerHTML += `<input type="text" name="payment[email]" value="${this.voucher.customer.email}"/>`;
      form.innerHTML += `<input type="text" name="payment[phone]" value="${this.voucher.customer.tel}"/>`;
      ...
      form.innerHTML += `<input type="text" name="payment[field3]" value='${this.voucher.shipping.id}'/>`;
  ...
  computed: {
    total() {
      var totalGift = 0;
      if (this.voucher.gifts.length > 0) {
        this.voucher.gifts.forEach((gift) => (totalGift += parseInt(gift.price)));
      }
      return totalGift;
    },
  },
